...
def save_model2(args, epoch, loss, model):
    # remove old models
    if epoch > 0:
        best_snapshot = 'best2_epoch_{}_loss_{:.8f}.pth'.format(
            args.best_record['epoch2'], args.best_record['loss2'])
        best_snapshot = os.path.join(args.exp_path, best_snapshot)
        assert os.path.exists(best_snapshot), 'cant find old snapshot {}'.format(best_snapshot)
        os.remove(best_snapshot)

    # save new best
    args.best_record['epoch2'] = epoch
    args.best_record['loss2'] = loss
