...
from models.traditionalML import rf_regression
import random
...
if __name__ == "__main__":
    ...
    # 训练SVR模型
    rfr_model = rf_regression(train_inputs, train_outputs, train_inputs, train_outputs)
    rfr_predictions = rfr_model.predict(val_inputs)

    print(rfr_predictions.shape)  # 预计输出 (51, 384*199)

    # 计算损失
    mean_abs_error_per_sample, max_abs_error_per_sample = compute_losses(val_outputs, rfr_predictions)
    ...
    for idx in random_sample_indices:
        ...
        prediction = rfr_predictions[idx].reshape(384,199)
