...
class CylinderflowDatasetLSTMBeta(Dataset):
    def __init__(self, data_path, train=True, train_ratio=0.8, random_points=False, num_points=16,
        ...
        self.data = data.reshape(-1,384*199)
        self.train = train
...
class cylinderflowdatasetMLP(Dataset):
    def __init__(self, data_path, train=True, train_ratio=0.8, random_points=False, num_points=16):
        """
        Custom dataset initializer.
        :param data_path: Path to the data (e.g., 'T' from your dataset)
        ...
        maxY = self.data.shape[2]

        # Select points in space
        if random_points:
            indices = np.random.randint(low=[0,0],high=[maxX-1,maxY-1],size=(num_points,2))
        else:
            x = np.linspace(0, maxX - 1, int(math.sqrt(num_points)), dtype=int)
            y = np.linspace(0, maxY - 1, int(math.sqrt(num_points)), dtype=int)
            xx,yy = np.meshgrid(x,y)
            indices = np.vstact([xx.ravel(),yy.ravel()]).T
        ...
        if self.train:
            timeslide = np.arange(self.num_train)
        else:
            timeslide = np.arange(self.num_train, self.length)
        self.data = self.data[timeslide,:,:]
    ...
    def __getitem__(self, idx):
        ...
        input = output[points[:,0],points[:,1]]
        output = output.flatten()
        return torch.tensor(input, dtype=torch.float32), torch.tensor(output, dtype=torch.float32)
class cylinderflowdatasetLSTM(Dataset):
    def __init__(self, data_path, train=True, train_ratio=0.8, random_points=False, num_points=16):
        """
        Custom dataset initializer.
        :param data_path: Path to the data (e.g., 'T' from your dataset)
