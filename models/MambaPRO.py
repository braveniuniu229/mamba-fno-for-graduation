...
class Block(nn.Module):
    def __init__(
        self, d_model,d_state, mixer_cls,
        ...
        self.mixer = mixer_cls(d_model,d_state)            # 这个是在参数里面定义的
...
def create_block(
    ...
    mixer_cls = lambda dim,d_state: Mamba(d_model=d_model, d_model_out=d_model,d_state=d_state)
    norm_cls=partial(
...
def create_finalblock(
    ...
    if ssm_cfg is None:
        ssm_cfg = {}
    factory_kwargs = {"device":device, "dtype":dtype}
    mixer_cls = lambda dim,d_state,d_model_out: Mamba(d_model=d_model, d_model_out=d_model_out,d_state=d_state)
    norm_cls=partial(
        nn.LayerNorm if not rms_norm else RMSNorm, eps=norm_epsilon, **factory_kwargs
    )
...
class MambaPRO(nn.Module):
    def __init__(self,
        ...
        self.finalBlock = create_finalblock(
            d_model,
            d_state,
            d_model_out,
            ssm_cfg=ssm_cfg,
...
def test():
    ...
    x = torch.randn(1,80,16).to(device)
    preds = model(x)
