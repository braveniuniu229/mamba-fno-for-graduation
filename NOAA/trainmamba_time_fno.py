...
from dataset.NOAAtemperature import NOAADatasetLSTMBeta,SameLengthBatchSampler
...
"""
args = parse_args()
args.arch = "MambaPOD_time_FNOinnerblock_standard"
args.d_model = args.num_points
args.d_model_out = 64800
args.expand = 2
...
file_mid = os.path.join(f"./experiment_log/{args.arch}_blocks_{args.num_blocks}_dstate_{args.d_state}_modesinner_{args.modes}_modesout_f{args.modes1}_width_f{args.width}", file_in)
...
train_dataset = NOAADatasetLSTMBeta(data_path=args.data_pth , train=True, slice_lengths=[150])
train_sampler = SameLengthBatchSampler(train_dataset.slices, batch_size=args.batch_size)
testdataset = NOAADatasetLSTMBeta(data_path=args.data_pth, train=False)
...
def test():
    ...
    with torch.no_grad():
        ...
        for inputs, outputs in testloader:
            ...
            pre_reshaped = pre.view(1, 383, 180, 360)
            outputs_reshaped = outputs.view(1, 383, 180, 360)

            for i in range(0, 383, 5):
